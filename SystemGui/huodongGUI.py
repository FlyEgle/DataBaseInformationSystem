...
from DataBaseInformationSystem.SystemGui.addDialog import HuoDongXinXiAddDataBase
from DataBaseInformationSystem.SystemGui.deleteDialog import HuodongDeleteDataBase
from DataBaseInformationSystem.SystemGui.queryDialog import HuoDongQueryDataBase
from DataBaseInformationSystem.SystemGui.sortDialog import HuodongSortDataBase
from DataBaseInformationSystem.SystemGui.updateDialog import HuodongxinxiUpdateDataBase


class TableHuoDongInfo(QWidget):
    def __init__(self):
        super(TableHuoDongInfo, self).__init__()
        self.sql_init()
        self.init_GUI()
    ...
    def init_GUI(self):
        ...
        self.titlelbl = QLabel('活动信息表', self)
        ...
        self.showTable.setHorizontalHeaderLabels(['activate_locate', 'activate_time', 'activate_content'])
    ...
    def sql_init(self):
        self.shetuandatabase_db = ConnectSQLdb('ACTIVATEDATABASE').sql_connect()
    ...
    def show_database(self):
        ...
        sql_db = ConnectSQLdb('ACTIVATEDATABASE').sql_connect()
        sql_cursor = sql_db.cursor()
        # 查询语句
        sql_query = 'SELECT * FROM activate_information;'
    ...
    def add_database(self):
        # 插入数据
        self.add_data_dialog = HuoDongXinXiAddDataBase()
        self.add_data_dialog.exec_()
        data = self.add_data_dialog.data_dict
        data_tuple = tuple([data['activate_locate'], data['activate_time'], data['activate_content']])
        print(data_tuple)
        sql_query = "INSERT INTO activate_information VALUES ('%s', '%s', '%s')"
    ...
    def delete_database(self):
        # 删除数据
        self.delete_data_dialog = HuodongDeleteDataBase()
        ...
        sql_query = "DELETE FROM activate_information where %s='%s'" % (str(data_index), str(data_value))
        try:
    ...
    def update_databse(self):
        # 固定两个字段，更新其他属性
        self.update_data_dialog = HuodongxinxiUpdateDataBase()
        self.update_data_dialog.exec_()
        # lianxifangshi = self.update_data_dialog.lianxifangshi_text
        context = self.update_data_dialog.context_text
        data_index = self.update_data_dialog.get_data_index
        data_value = self.update_data_dialog.get_data_value
        sql_query = "UPDATE activate_information SET activate_content='%s' WHERE %s='%s'" % (context, data_index, data_value)
        try:
            self.shetuandatabase_cursor.execute(sql_query)
            self.shetuandatabase_db.commit()
        except:
    ...
    def query_database(self):
        # 查询出来的结果显示在tabel上
        self.query_data_dialog = HuoDongQueryDataBase()
        ...
        sql_query = "SELECT * FROM activate_information WHERE %s='%s'" %(str(data_index), str(data_value))
        try:
            self.shetuandatabase_cursor.execute(sql_query)
            self.shetuandatabase_db.commit()
    ...
    def sort_database(self):
        self.sort_data_dialog = HuodongSortDataBase()
        self.sort_data_dialog.exec_()
        data_index = self.sort_data_dialog.get_data_index
        sql_query = "SELECT * from activate_information ORDER BY %s ASC" %(str(data_index))
        try:
            self.shetuandatabase_cursor.execute(sql_query)
            self.shetuandatabase_db.commit()
            data = self.shetuandatabase_cursor.fetchall()
