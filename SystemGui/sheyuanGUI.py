...
from DataBaseInformationSystem.SystemGui.addDialog import SheyunAddDatabase
from DataBaseInformationSystem.SystemGui.deleteDialog import SheYuanDeleteDataBase
from DataBaseInformationSystem.SystemGui.queryDialog import SheyuanQueryDataBase
from DataBaseInformationSystem.SystemGui.sortDialog import SheYuanSortDataBase
from DataBaseInformationSystem.SystemGui.updateDialog import SheyuanInfoUpdateDataBase


class TableSheYuanInfo(QWidget):
    def __init__(self):
        super(TableSheYuanInfo, self).__init__()
        self.sql_init()
        self.init_GUI()
    ...
    def init_GUI(self):
        ...
        self.titlelbl = QLabel('社员信息表', self)
        ...
        self.showTable.setHorizontalHeaderLabels(['person_id', 'person_name',
                                                  'person_sex', 'person_locate', 'person_iphone'])
    ...
    def sql_init(self):
        self.shetuandatabase_db = ConnectSQLdb('SHEYUANDATABASE').sql_connect()
    ...
    def show_database(self):
        ...
        sql_db = ConnectSQLdb('SHEYUANDATABASE').sql_connect()
        sql_cursor = sql_db.cursor()
        # 查询语句
        sql_query = 'SELECT * FROM person_information;'
    ...
    def add_database(self):
        # 插入数据
        self.add_data_dialog = SheyunAddDatabase()
        self.add_data_dialog.exec_()
        data = self.add_data_dialog.data_dict
        data_tuple = tuple([data['person_id'], data['person_name'],
                      data['person_sex'], data['person_locate'], data['person_iphone']])
        print(data_tuple)
        sql_query = "INSERT INTO person_information VALUES ('%s', '%s', '%s', '%s', '%s')"
    ...
    def delete_database(self):
        # 删除数据
        self.delete_data_dialog = SheYuanDeleteDataBase()
        ...
        if type(data_value) == str:
            sql_query = "DELETE FROM person_information where %s='%s'" %( data_index, data_value)
        else:
            sql_query = "DELETE FROM person_information where %s=%s" % (data_index, data_value)
        try:
    ...
    def update_databse(self):
        # 固定两个字段，更新其他属性
        self.update_data_dialog = SheyuanInfoUpdateDataBase()
        ...
        if type(data_value) == str:
            sql_query = "UPDATE person_information SET person_locate='%s' WHERE %s='%s'" % (locate, data_index, data_value)
        else:
            sql_query = "UPDATE person_information SET person_locate='%s' WHERE %s=%s" % (
        ...
        try:
            ...
            self.shetuandatabase_db.rollback()
        if type(data_value) == str:
            sql_query2 = "UPDATE person_information SET person_iphone='%s' WHERE %s='%s'" % (lianxifangshi, data_index, data_value)
        else:
            sql_query2 = "UPDATE person_information SET person_iphone='%s' WHERE %s=%s" % ( lianxifangshi, data_index, data_value)
        try:
    ...
    def query_database(self):
        # 查询出来的结果显示在tabel上
        self.query_data_dialog = SheyuanQueryDataBase()
        ...
        if type(data_value) == str:
            sql_query = "SELECT * FROM person_information WHERE %s='%s'" %(data_index, data_value)
        else:
            sql_query = "SELECT * FROM person_information WHERE %s=%s" % (data_index, data_value)
        try:
            self.shetuandatabase_cursor.execute(sql_query)
    ...
    def sort_database(self):
        self.sort_data_dialog = SheYuanSortDataBase()
        self.sort_data_dialog.exec_()
        data_index = self.sort_data_dialog.get_data_index
        sql_query = "SELECT * from person_information ORDER BY %s ASC" %(str(data_index))
        try:
            self.shetuandatabase_cursor.execute(sql_query)
            self.shetuandatabase_db.commit()
            data = self.shetuandatabase_cursor.fetchall()
