...
from DataBaseInformationSystem.SystemGui.addDialog import ShetuantuanzhangAddDataBase
from DataBaseInformationSystem.SystemGui.deleteDialog import SheTuanTuanZhangDeleteDataBase
from DataBaseInformationSystem.SystemGui.queryDialog import SheTuanTuanZhangQueryDataBase
from DataBaseInformationSystem.SystemGui.sortDialog import SheTuanTuanZhangSortDataBase
from DataBaseInformationSystem.SystemGui.updateDialog import ShetuantuanzhangUpdateDataBase


class TableSheTuanTuanZhangInfo(QWidget):
    def __init__(self):
        super(TableSheTuanTuanZhangInfo, self).__init__()
        self.sql_init()
        self.init_GUI()
    ...
    def init_GUI(self):
        ...
        self.titlelbl = QLabel('社团团长信息表', self)
        ...
        self.showTable.setHorizontalHeaderLabels(['shetuantuanzhang_id', 'shetuantuanzhang_name',
                                                  'shetuantuanzhang_sex', 'shetuantuanzhang_locate',
                                                  'shetuantuanzhang_iphone'])
    ...
    def sql_init(self):
        self.shetuandatabase_db = ConnectSQLdb('SHETUANTUANZHANGDATABASE').sql_connect()
    ...
    def show_database(self):
        ...
        sql_db = ConnectSQLdb('SHETUANTUANZHANGDATABASE').sql_connect()
        sql_cursor = sql_db.cursor()
        # 查询语句
        sql_query = 'SELECT * FROM shetuantuanzhang_information;'
    ...
    def add_database(self):
        # 插入数据
        self.add_data_dialog = ShetuantuanzhangAddDataBase()
        self.add_data_dialog.exec_()
        data = self.add_data_dialog.data_dict
        data_tuple = tuple([data['shetuantuanzhang_id'], data['shetuantuanzhang_name'],
                      data['shetuantuanzhang_sex'], data['shetuantuanzhang_locate'],
                            data['shetuantuanzhang_iphone']])
        print(data_tuple)
        sql_query = "INSERT INTO shetuantuanzhang_information VALUES ('%s', '%s', '%s', '%s', '%s')"
    ...
    def delete_database(self):
        # 删除数据
        self.delete_data_dialog = SheTuanTuanZhangDeleteDataBase()
        ...
        if type(data_value) == str:
            sql_query = "DELETE FROM shetuantuanzhang_information where %s='%s'" %(data_index, data_value)
        else:
            sql_query = "DELETE FROM shetuantuanzhang_information where %s=%s" % (data_index, data_value)
        try:
    ...
    def update_databse(self):
        # 固定两个字段，更新其他属性
        self.update_data_dialog = ShetuantuanzhangUpdateDataBase()
        self.update_data_dialog.exec_()
        lianxifangshi = self.update_data_dialog.lianxifangshi_text
        suozaixi = self.update_data_dialog.suozaixi_text
        data_index = self.update_data_dialog.get_data_index
        data_value = self.update_data_dialog.get_data_value
        sql_query = "UPDATE shetuantuanzhang_information SET shetuantuanzhang_locate='%s' WHERE %s='%s'" % (suozaixi,
                                                                                                         data_index,
                                                                                                         data_value)
        try:
            self.shetuandatabase_cursor.execute(sql_query)
            ...
            self.shetuandatabase_db.rollback()
        sql_query2 = "UPDATE shetuantuanzhang_information SET shetuantuanzhang_iphone='%s' WHERE %s='%s'" % (lianxifangshi,
                                                                                                       data_index,
                                                                                                       data_value)
        try:
    ...
    def query_database(self):
        # 查询出来的结果显示在tabel上
        self.query_data_dialog = SheTuanTuanZhangQueryDataBase()
        ...
        if type(data_value) == str:
            sql_query = "SELECT * FROM shetuantuanzhang_information WHERE %s='%s'" %(str(data_index), str(data_value))
        else:
            sql_query = "SELECT * FROM shetuantuanzhang_information WHERE %s=%s" % (str(data_index), str(data_value))
        try:
            self.shetuandatabase_cursor.execute(sql_query)
    ...
    def sort_database(self):
        self.sort_data_dialog = SheTuanTuanZhangSortDataBase()
        self.sort_data_dialog.exec_()
        data_index = self.sort_data_dialog.get_data_index
        sql_query = "SELECT * from shetuantuanzhang_information ORDER BY %s ASC" %(str(data_index))
        try:
            self.shetuandatabase_cursor.execute(sql_query)
            self.shetuandatabase_db.commit()
            data = self.shetuandatabase_cursor.fetchall()
...
if __name__ == '__main__':
    ...
    tableshow = TableSheTuanTuanZhangInfo()
    tableshow.show()
    sys.exit(app.exec_())
