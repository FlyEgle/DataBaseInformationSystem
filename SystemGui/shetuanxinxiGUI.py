...
from DataBaseInformationSystem.SystemGui.addDialog import ShetuanxinxiAddDatabase
from DataBaseInformationSystem.SystemGui.deleteDialog import SheTuanInfoDeleteDataBase
from DataBaseInformationSystem.SystemGui.queryDialog import SheTuanInfoQueryDataBase
from DataBaseInformationSystem.SystemGui.sortDialog import SheTuanXinXiSortDataBase
from DataBaseInformationSystem.SystemGui.updateDialog import SheTuanInfoUpdateDataBase


class TableShetuanInfo(QWidget):
    def __init__(self):
        super(TableShetuanInfo, self).__init__()
        self.sql_init()
        self.init_GUI()
    ...
    def init_GUI(self):
        ...
        self.titlelbl = QLabel('社团信息', self)
        ...
        self.showTable.setHorizontalHeaderLabels(['association_id', 'association_name',
                                                  'association_location', 'association_phone'])
    ...
    def sql_init(self):
        self.shetuandatabase_db = ConnectSQLdb('SHETUANXINXIDATABASE').sql_connect()
    ...
    def show_database(self):
        ...
        # 查询语句
        sql_query = 'SELECT * FROM association_information;'
    ...
    def add_database(self):
        # 插入数据
        self.add_data_dialog = ShetuanxinxiAddDatabase()
        self.add_data_dialog.exec_()
        data = self.add_data_dialog.data_dict
        data_tuple = tuple([data['association_id'], data['association_name'],
                      data['association_location'], data['association_phone']])
        print(data_tuple)
        sql_query = "INSERT INTO association_information VALUES ('%s', '%s', '%s', '%s')"
    ...
    def delete_database(self):
        # 删除数据
        self.delete_data_dialog = SheTuanInfoDeleteDataBase()
        ...
        if type(data_value) == str:
            sql_query = "DELETE FROM association_information where %s='%s'" % (data_index, data_value)
        else:
            sql_query = "DELETE FROM association_information where %s=%s" % (data_index, data_value)
        try:
    ...
    def update_databse(self):
        # 固定两个字段，更新其他属性
        self.update_data_dialog = SheTuanInfoUpdateDataBase()
        ...
        if type(data_value) == str:
            sql_query = "UPDATE association_information SET association_location='%s' WHERE %s='%s'" % (locate, data_index, data_value)
        else:
            sql_query = "UPDATE association_information SET association_location='%s' WHERE %s=%s" % (
            locate, data_index, data_value)
        try:
            self.shetuandatabase_cursor.execute(sql_query)
            self.shetuandatabase_db.commit()
        except:
            self.shetuandatabase_db.rollback()
        if type(data_value) == str:
            sql_query2 = "UPDATE association_information SET association_phone='%s' WHERE %s='%s'" % (lianxifangshi, data_index, data_value)
        else:
            sql_query2 = "UPDATE association_information SET association_phone='%s' WHERE %s=%s" % (
            lianxifangshi, data_index, data_value)
        try:
    ...
    def query_database(self):
        # 查询出来的结果显示在tabel上
        self.query_data_dialog = SheTuanInfoQueryDataBase()
        ...
        if type(data_value) == str:
            sql_query = "SELECT * FROM association_information WHERE %s='%s'" %(data_index, data_value)
        else:
            sql_query = "SELECT * FROM association_information WHERE %s=%s" %(data_index, data_value)
        try:
            self.shetuandatabase_cursor.execute(sql_query)
    ...
    def sort_database(self):
        self.sort_data_dialog = SheTuanXinXiSortDataBase()
        self.sort_data_dialog.exec_()
        data_index = self.sort_data_dialog.get_data_index
        sql_query = "SELECT * from association_information ORDER BY %s ASC" %(str(data_index))
        try:
            self.shetuandatabase_cursor.execute(sql_query)
            self.shetuandatabase_db.commit()
            data = self.shetuandatabase_cursor.fetchall()
